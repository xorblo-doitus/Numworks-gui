...
EMULATED=_A
try:import os;EMULATED=_B;print('Emulated')
except:pass
...
class ColorName(str):
	...
	def specified(self,variant,enabled=_B):
		if enabled:return ColorName(self+_VARIANT_DELIMITER+variant)
		return self
	def uneditable(self,enabled=_B):return self.specified('uneditable',enabled)
	def hovered(self,enabled=_B):return self.specified('hovered',enabled)
	def enabled(self,enabled=_B):return self.specified('enabled',enabled)
	def focused(self,enabled=_B):return self.specified('focused',enabled)
...
class Style:
	...
	def __init__(self,fallback=_C,colors={}):
...
class CanvasItem:
	def __init__(self,position=Vector2(10,10),callback=lambda:_C,style=_C):self.position=position;self.style=style;self.callback=callback
	def draw(self,pos=_C):'Virtual';raise NotImplementedError('draw() is not implemented on '+repr(self))
...
class Label(Hoverable):
	...
	def draw(self,pos=_C):pos=pos or self.position;size=add_overlay(self.get_size());fill_rect(pos.x-1,pos.y-1,size.x,size.y,self.get_color(Colors.border));draw_string(self.txt,pos.x,pos.y,self.get_color(Colors.text),self.get_color(Colors.background))
...
class TextBox(Focusable):
	...
	def draw(self,pos=_C):
		...
		fill_rect(pos.x-1,pos.y-1,size_with_overlay.x,size_with_overlay.y,self.get_color(Colors.border));fill_rect(pos.x,pos.y,size.x,size.y,(255,255,255));draw_string(self.txt[offset:min(offset+self.size,len(self.txt))],pos.x,pos.y,(0,0,0),(255,255,255))
...
class Slider(Focusable):
	...
	def __init__(self,min,max,step=1,initial_value=_C,size=100,*args,**kwargs):super().__init__(*args,**kwargs);self.min=min;self.max=max;self.step=step;self.value=round((min+max)/2/step)*step if initial_value==_C else initial_value;self.size=size
...
def start():
	...
	for row in layout:
		...
		row[0].enabled=_B
	layout_get((0,0)).hovered=_B;y=111-canvas_items_height(layout)//2
	for row in layout:
	...
	while not keydown(KEY_OK):
		sleep(1/TPS);old_hovering_pos=_C
		if focused:hovering_canvas_item.handle_input()
			...
			if old_hovering_pos and old_hovering_pos!=hovering_pos:button=layout_get(old_hovering_pos);button.hovered=_A;button.draw();hovering_canvas_item.hovered=_B;hovering_canvas_item.draw()
		if keydown(KEY_EXE):
			if isinstance(hovering_canvas_item,Focusable):focused=not focused;hovering_canvas_item.focused=focused;hovering_canvas_item.draw()
				...
				for(i,canvas_item)in enumerate(layout[hovering_pos.y]):
					if isinstance(canvas_item,Button):
						if i==hovering_pos.x:
							if not canvas_item.enabled:canvas_item.enabled=_B;canvas_item.draw()
		...
		while old_hovering_pos:
			old_hovering_pos=_A
			for key in MOVES:
				if keydown(key):old_hovering_pos=_B
